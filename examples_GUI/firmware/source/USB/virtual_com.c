 ...
 * @return None.
 */
parser_status_t USBTask(parse_vcom_t *vcom_msg)
{
    usb_status_t error = kStatus_USB_Error;
    parser_status_t parsing_status = CMD_NOT_RECEIVED;
    ...
    if ((1U == s_cdcVcom.attach) && (1U == s_cdcVcom.startTransactions))
    {
        ...
        if ((0 != s_recvSize) && (USB_CANCELLED_TRANSFER_LENGTH != s_recvSize))
        {
            ...
            if ((0U != s_recvSize) && (USB_CANCELLED_TRANSFER_LENGTH != s_recvSize))
            {
                ...
                if(s_currRecvBuf[0]==START_BYTE_VCOM){
                	// Conformity of the start byte
                	sizeMsg = s_currRecvBuf[1];

                	if(s_currRecvBuf[sizeMsg+3]==END_BYTE_VCOM){
                		// Conformity of the end byte, massage can be processed
                		vcom_msg->itype = s_currRecvBuf[2];
                		vcom_msg->imsg =(uint8_t *) calloc(sizeMsg,sizeof(uint8_t));
                		memcpy(vcom_msg->imsg,&s_currRecvBuf[3],sizeMsg);
                		vcom_msg->isize = sizeMsg;

                		parsing_status = CMD_RECEIVED;
                		 s_recvSize = 0;
                	}
                	else{
                		// No Conformity of the end byte
                		parsing_status = CMD_RECEIVED_WITH_PROTOCOL_ERROR;
                		 s_recvSize = 0;
                	}
                }
                else{
                	// No Conformity of the start byte
                	parsing_status = CMD_RECEIVED_WITH_PROTOCOL_ERROR;
                	 s_recvSize = 0;
                }
            }
        ...
        if (0U != vcom_msg->osize)
        {
        	// There is a message to send at host.
            uint32_t size = vcom_msg->osize;
            vcom_msg->osize    = 0;

            s_currSendBuf[0] = START_BYTE_VCOM;
            s_currSendBuf[1] = size;
            s_currSendBuf[2] = vcom_msg->otype;
            memcpy(&s_currSendBuf[3],vcom_msg->omsg,size);
            s_currSendBuf[size+3] = END_BYTE_VCOM;
            free(vcom_msg->omsg);

            error = USB_DeviceCdcAcmSend(s_cdcVcom.cdcAcmHandle, USB_CDC_VCOM_BULK_IN_ENDPOINT, s_currSendBuf, size + 4);

            parsing_status = CMD_SENT;

            if (error != kStatus_USB_Success)
            {
                /* Failure to send Data Handling code here */
            	parsing_status = USB_ERROR;
            }
        }
