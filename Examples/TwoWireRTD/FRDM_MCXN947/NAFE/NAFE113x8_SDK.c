 ...
 */
/*
 *file NAFE113x8_SDK.c
 */

#include "NAFE113x8_Register.h"
#include "NAFE113x8_SDK.h"

static void NAFE113x8_CaptureSamplesSCCR(uint16_t count);
...
static void NAFE113x8_CaptureSamplesMCCR(uint16_t activeChannelCount, bool dataOut16, bool statusByteEn, uint16_t count){
	uint16_t sampleCount = 0;
	while (sampleCount <count){
		if (SampleReceived){
			SampleReceived=false;
...
void NAFE113x8_doSCCR(uint8_t channel, bool devID, uint16_t count){
    ...
    NAFE113x8_CaptureSamplesSCCR(count);
...
void NAFE113x8_doMCCR(bool devID, uint16_t count){
    ...
    NAFE113x8_CaptureSamplesMCCR(activeChannelCount,false,false,count);
