...
/*
 * @file  voltageSensing_SCSR.c
 * @brief The voltageSensing_SCSR.c file implements
...
/*!@brief        Function to sense voltage in SCSR mode.
 *  @details     This is the function to sense voltage across different analog inputs in SCSR conversion mode.
 *  @constraints None
 *
 *  @reentrant   No
 *  @return      No
...
static void VSenseSCSR(){
	...
	nafe13388_scsr_conversion vs_scsr;
	...
	NAFE13388_SCSR(&nafe13388Driver, ACTIVE_CHANNELS , gains,&vs_scsr);

	for(int channel = 0; channel < ACTIVE_CHANNELS; channel++)
	{
		if (vs_scsr.conversionResult[channel] < 0.0f)
			PRINTF("\r\n\033[32mChannel %d = -%.6fV\033[37m\r\n",channel,fabs(vs_scsr.conversionResult[channel]));
		else
			PRINTF("\r\n\033[32mChannel %d = %.6fV\033[37m\r\n",channel,vs_scsr.conversionResult[channel]);
		PRINTF("\r\nElapsed Time for Channel %d = %3fms\r\n", channel ,vs_scsr.time[channel]);
	}
