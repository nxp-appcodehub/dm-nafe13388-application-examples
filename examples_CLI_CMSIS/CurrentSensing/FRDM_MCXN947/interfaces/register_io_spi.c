...
#if NAFE13388_UIM
/*! The interface function to write to a device directly. */
int32_t Command_SPI_Write(ARM_DRIVER_SPI *pCommDrv,
                          registerDeviceInfo_t *devInfo,
                          void *pWriteParams,
                          uint16_t command,
                          uint32_t size)
{
    ...
    // Configure slave select (SS) commands for enabling and disabling SPI communication
    spiControlParams_t ss_en_cmd = {
        .cmdCode = ARM_SPI_SS_ACTIVE,
        .activeValue = pSlaveParams->ssActiveValue,
        .pTargetSlavePinID = pSlaveParams->pTargetSlavePinID,
    ...
    // Prepare the command-only SPI transaction
    pSlaveParams->pCommandPreprocessFN(&slaveWriteCmd, size, slaveWriteCmd.pWriteBuffer, command);  // Command pre-processing
    ...
    // Perform the SPI transfer to send the command
    status = pCommDrv->Transfer(slaveWriteCmd.pWriteBuffer, slaveWriteCmd.pReadBuffer, slaveWriteCmd.size);
    if (ARM_DRIVER_OK == status)
    {
