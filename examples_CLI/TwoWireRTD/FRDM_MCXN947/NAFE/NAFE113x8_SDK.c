...
static uint8_t NAFE113x8_CaptureSamplesSCCR(uint16_t count);
static void NAFE113x8_CaptureSamplesMCMR(uint16_t activeChannelCount, bool dataOut16, bool statusByteEn);
void NAFE113x8_DELAY_ms(uint32_t delay_ms);
...
static uint8_t NAFE113x8_CaptureSamplesSCCR(uint16_t count){
	...
	while (sampleCount <count){
		/* Delay of 1 second to wait for sample received flag to be set */
		NAFE113x8_DELAY_ms(1000);
		...
		if(SampleReceived){
			...
			ret = TIMEOUT;
			PRINTF("\r\nSample not received, Trying Again!!\r\n");
			break;
		}
	}
	return ret;
...
static uint8_t NAFE113x8_CaptureSamplesMCCR(uint16_t activeChannelCount, bool dataOut16, bool statusByteEn, uint16_t count){
	uint16_t sampleCount = 0;
	int ret = SUCCESS;
...
uint8_t NAFE113x8_doSCCR(uint8_t channel, bool devID, uint16_t count){
	...
	int ret;
	ret = NAFE113x8_CaptureSamplesSCCR(count);
...
void NAFE113x8_doMCMR(bool devID){
	...
	NAFE113x8_CaptureSamplesMCMR(activeChannelCount,false,false);
...
uint8_t NAFE113x8_doMCCR(bool devID, uint16_t count){
	...
	int ret; 
	ret = NAFE113x8_CaptureSamplesMCCR(activeChannelCount,false,false,count);
